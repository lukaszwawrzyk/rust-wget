...
impl Progress {
  ...
  pub fn try_initialize_sized(&mut self, size: u64) -> () {
    self.try_initialize(Some(size_to_download + self.predownloaded_size.unwrap_or(0)));
  }
  ...
  fn show_status(&self) -> () {
    match self.total_size {
        Some(total_size) => {
            ...
            let bytes_left = bytes_total - bytes_read - self.predownloaded_size;
    ...
    fn status_bar(&self, time_elapsed: u64) -> String {
      ...
      let indicator_center_position = math::min(math::abs((time_elapsed % (PROGRESS_BAR_SIZE * 2)) - PROGRESS_BAR_SIZE + 1), PROGRESS_BAR_SIZE - 1);

      bar_buffer[indicator_center_position as usize] = '+';
      if (indicator_center_position - 1 >= 0) {
  ...
  fn progress_bar(&self, progress_percent: f32) -> String {
    let mut bar_buffer: [char; PROGRESS_BAR_SIZE] = [' '; PROGRESS_BAR_SIZE];
    let predownloaded_percent = 100f32 * safe_div_f32(self.predownloaded_size as f32, self.total_size.unwrap() as f32);
    ...
    let (new_fill_start, new_fill_end) = if last_to_fill_predownloaded >= 0 and last_to_fill_new_progress >= 0 {
